...
describe('HeatMapData', function () {
    ...
    describe('getTitle()', function () {
        it('Default title should be empty', function () {
            var hmd = new HeatMapData({start:1,end:2, value:3});
            var hmdX = hmd.getTitle();
            assert.equal(hmdX, '');
        });
        it('Default title should be a string when includeTitle is true', function () {
            var hmd = new HeatMapData({start:1,end:2, value:3}, 0, true);
    ...
    describe('buildCellArray()', function () {
        it('should should be sorted by value low to high', function () {
            ...
            var inputArray = [{start:1,end:2, value:1}, {start:4,end:5, value:0}];
            var result = HeatMapData.buildCellArray(inputArray, {
                xOffset: 0,
                includeTitle: true,
                scale: 1,
                height: 10,
        ...
        it('overlapping items should share titles seperated by newline', function () {
            ...
            assert.equal(result.length, 2);
            assert.equal(result[0].title, '1-5 : 1\n4-7 : 0');
            assert.equal(result[1].title, '1-5 : 1\n4-7 : 0');
        });
        ...
        it('overlapping items should not combine everything', function () {
            ...
            assert.equal(result[0].title, '713911-713931 : 0.215');
            assert.equal(result[1].title, '714000-714020 : 0.2865\n714002-714022 : 0.3133');
            assert.equal(result[2].title, '714157-714177 : 0.2967');
            assert.equal(result[3].title, '714000-714020 : 0.2865\n714002-714022 : 0.3133');
            assert.equal(result[4].title, '713975-713995 : 0.3168');
            assert.equal(result[5].title, '714050-714070 : 0.3345');
        });
